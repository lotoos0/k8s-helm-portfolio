...
venv: ## [Setup] Create virtualenv
	@test -d $(VENV) || $(PY) -m venv $(VENV)

install: venv ## [Setup] Install all dependencies (API + worker + scripts)
	$(PIP) install --upgrade pip
	$(PIP) install -r api/requirements.txt
	$(PIP) install -r worker/requirements.txt
	$(PIP) install -r scripts/requirements.txt

run-api: ## [Dev] Run FastAPI locally on :800 (reload)
	$(VENV)/bin/uvicorn api.app.main:app --host 0.0.0.0 --port 8000 --reload

run-worker: ## [Dev] Run Celery workder ( requires Redis running )
	cd worker && ../$(VENV)/bin/celery -A app.celery_app.app worker -l info

clean: ## [Setup] Remove cache
...
build-api: ## [Docker] Build API Docker image
	docker build -t $(IMAGE_API) ./api

run-api-docker: ## [Docker] Run API container on :8000
	- docker rm -f $(CONTAINER_API) >/dev/null 2>&1 || true
	docker run -d --name $(CONTAINER_API) -p 8000:8000 $(IMAGE_API)

logs-api: ## [Docker] Tail logs from API container
	docker logs -f $(CONTAINER_API)

stop-api: ## [Docker] Stop and remove API container
	- docker rm -f $(CONTAINER_API) >/dev/null 2>&1 || true

# --------- Docker Compose ----------
compose-up: ## [Docker] Build & run all services (api, redis, worker)
	docker compose up -d --build

compose-logs: ## [Docker] Tail logs of all services
	docker compose logs -f

compose-down: ## [Docker] Stop and remove all services
	docker compose down -v

# ---------- Smoke helpers (Celery calls via docker exec) ---------
worker-ping: ## [Worker] Call ping task on worker (result in logs)
	docker exec october-worker python -c "from worker.app.tasks import ping; print(ping.delay().id)"

worker-add: ## [Worker] Call add(1,2) task on worker (result id printed)
...
worker-result: ## [Worker] Get result by id: make worker-result ID=<task_id>
...
# ----------- Dev quality ------------
lint: ## [Testing] Run Ruff (lint + imports)
	$(VENV)/bin/ruff check api

fmt: ## [Testing] Format with Black + Ruff imports
	$(VENV)/bin/black api
	$(VENV)/bin/ruff check --select I --fix api

test: ## [Testing] Run pytest with coverage
	$(VENV)/bin/pytest --cov=api --cov-report=term-missing

ci: ## [Testing] Lint + Test (for local CI-like run)
...
k8s-build-load: ## [Kubernetes] Build images and load to Minikube
	make build-api
	minikube image load october-api:dev

minikube-cleanup-images: ## [Kubernetes] Remove old dev-* images from Minikube (keeps last 3)
...
dev-deploy: ## [Dev] Quick dev: build + load + helm upgrade (uses timestamp tag)
	@TAG=dev-$$(date +%s); \
...
dev-deploy-quick: ## [Dev] Quick deploy: build + kubectl set image (faster, no helm)
	@TAG=dev-$$(date +%s); \
	echo "Deploying with tag: $$TAG"; \
	docker build -t october-api:$$TAG ./api; \
	minikube image load october-api:$$TAG; \
...
k8s-apply: ## [Kubernetes] Apply namespace + config + api (deployment, service)
...
k8s-delete: ## [Kubernetes] Delete all app resources
...
k8s-port-api: ## [Kubernetes] Port-forward API :8080 -> svc/api:80
	kubectl -n $(KNS) port-forward svc/api 8080:80

k8s-logs-api: ## [Kubernetes] Tail API pod logs
	kubectl -n $(KNS) logs -l app=api -f --max-log-requests=3

k8s-describe-api: ## [Kubernetes] Describe API deployment and pod
	kubectl -n $(KNS) describe deploy/api
	kubectl -n $(KNS) get po -l app=api
	kubectl -n $(KNS) describe po -l app=api | sed -n '1,120p'

k8s-restart-api: ## [Kubernetes] Rollout restart API
	kubectl -n $(KNS) rollout restart deploy/api
	kubectl -n $(KNS) rollout status deploy/api

k8s-set-image-api: ## [Kubernetes] Update image (usage: make k8s-set-image-api IMG=myrepo/october-api:tag)
	@if [ -z "$(IMG)" ]; then echo "Usage: make k8s-set-image-api IMG=<image>"; exit 1; fi
	kubectl -n $(KNS) set image deploy/api api=$(IMG)
	kubectl -n $(KNS) rollout status deploy/api

k8s-get: ## [Kubernetes] Quick view
	kubectl -n $(KNS) get deploy,po,svc

# ------------- Ingress (MInikube) -----------

k8s-enable-ingress: ## [Kubernetes] Enable NGINX ingress addon in Minikube
	minikube addons enable ingress
	kubectl -n ingress-nginx get pods

k8s-apply-ingress: ## [Kubernetes] Apply API ingress with dynamic host api.<minikube-ip>.nip.io
	@IP=$$(minikube ip); \
	  HOST=api.$$IP.nip.io; \
	  echo "Using host: $$HOST"; \
	  cat deploy/k8s/ingress-api.yaml | sed "s/api\.[0-9.]*\.nip\.io/$$HOST/g" | kubectl apply -f -

k8s-delete-ingress: ## [Kubernetes] Delete API ingress (host-insensitive)
	- kubectl -n $(KNS) delete ingress/api --ignore-not-found

k8s-curl-ingress: ## [Kubernetes] Curl ingress /healthz
	@IP=$$(minikube ip); HOST=api.$$IP.nip.io; \
	  echo "GET http://$$HOST/healthz"; \
	  curl -sS --max-time 5 http://$$HOST/healthz | jq .

k8s-open-ingress: ## [Kubernetes] Open in browser (Linux xdg-open/mac open)
...
# ----- Metrics server / HPA ----------
k8s-enable-metrics: ## [Kubernetes] Enable metrics-server in Minikube
	minikube addons enable metrics-server
	kubectl -n kube-system get pods | grep metrics-server || true

k8s-apply-hpa: ## [Kubernetes] Apply HPA for API
	kubectl apply -f deploy/k8s/hpa-api.yaml

k8s-hpa-status: ## [Kubernetes] Watch HPA & ReplicaSet status
	@echo "HPA:"
...
k8s-top: ## [Kubernetes] Show CPU/mem usage for pods and nodes
	@echo "Pods usage:"; kubectl -n $(KNS) top pods || true
	@echo "Nodes usage:"; kubectl top nodes || true

k8s-break-liveness: ## [Kubernetes] Break liveness probe (set /boom)
	kubectl -n october patch deploy api --type='json' \
	  -p='[{"op":"replace","path":"/spec/template/spec/containers/0/livenessProbe/httpGet/path","value":"/boom"}]'

k8s-fix-liveness: ## [Kubernetes] Restore liveness probe (/healthz) & wait
	kubectl -n october patch deploy api --type='json' \
...
# ---------- Load tester -----------
load-test: ## [Testing] Run HTTP load test (use URL=... CONC=... DUR=...)
...
# --------- K8s: Redis & Worker -----------
k8s-apply-redis: ## [Kubernetes] Apply Redis (PVC + Deployment + Service)
...
k8s-apply-worker: ## [Kubernetes] Apply Celery worker deployment
	kubectl apply -f deploy/k8s/worker/deployment.yaml
	kubectl -n $(KNS) rollout status deploy/worker

k8s-logs-worker: ## [Kubernetes] Tail worker logs
	kubectl -n $(KNS) logs -l app=worker -f --max-log-requests=3

# Exec helpers: run python in worker pod
k8s-exec-worker-ping: ## [Kubernetes] Call ping() via python inside worker pod
...
k8s-exec-worker-add: ## [Kubernetes] Call add(1,2) via python inside worker pod
	@POD=$$(kubectl -n $(KNS) get po -l app=worker -o jsonpath='{.items[0].metadata.name}'); \
...
helm-lint: ## [Helm] Helm lint (strict)
	helm lint $(HELM_DIR) --strict

helm-template-dev: ## [Helm] Render templates (dev, validate)
	@IP=$$(minikube ip); HOST=api.$$IP.nip.io; \
...
helm-up-dev: ## [Helm] Upgrade/Install dev (Minikube)
	@IP=$$(minikube ip); HOST=api.$$IP.nip.io; \
	  helm upgrade --install $(HELM_NAME) $(HELM_DIR) \
...
helm-del: ## [Helm] Uninstall release
	helm uninstall $(HELM_NAME) -n $(HELM_NS) || true

helm-history: ## [Helm] Show release history
	helm history $(HELM_NAME) -n $(HELM_NS) || true

helm-diff-dev: ## [Helm] Show diff vs current (dev)
	@IP=$$(minikube ip); HOST=api.$$IP.nip.io; \
...
helm-rollback: ## [Helm] Rollback to a given REV (use: make helm-rollback REV=2)
...
push-all: ## [CI/CD] Push both images
...
cd-dev: ## [CI/CD] Helm upgrade/install to dev using local kubeconfig (override DEV_HOST, images)
	helm upgrade --install $(HELM_NAME) $(HELM_DIR) \
...
helm-up-dev-atomic: ## [Helm] Helm upgrade/install (atomic, timeout)
	@IP=$$(minikube ip); HOST=api.$$IP.nip.io; \
	  helm upgrade --install $(HELM_NAME) $(HELM_DIR) \
	    -n $(HELM_NS) --create-namespace \
	    -f $(HELM_DIR)/values.yaml -f $(HELM_DIR)/values-dev.yaml \
...
helm-rollback-last: ## [Helm] Rollback to previous revision
	@REV=$$(helm history $(HELM_NAME) -n $(HELM_NS) | awk 'NR==2{print $$1}'); \
	  echo "Rolling back to $$REV"; \
	  helm rollback $(HELM_NAME) $$REV -n $(HELM_NS); \
	  helm history $(HELM_NAME) -n $(HELM_NS)

cd-dev-atomic: ## [CI/CD] Local CD (atomic) with images from GHCR (override ORG/REGISTRY/TAGS)
	@IP=$$(minikube ip); HOST=api.$$IP.nip.io; \
	  helm upgrade --install $(HELM_NAME) $(HELM_DIR) \
	    -n $(HELM_NS) --create-namespace \
	    -f $(HELM_DIR)/values.yaml -f $(HELM_DIR)/values-dev.yaml \
...
smoke-ci: ## [Testing] Run E2E smoke (Ingerss) using scripts/smoke.sh; override HOST=...
	@if [ -z "$(HOST)" ]; then echo "Usage: make smoke-ci HOST=api.<minikube-ip>.nip.io"; exit 2; fi
	./scripts/smoke.sh "$(HOST)"

smoke-pf: ## [Testing] Run E2E via port-forward (local fallback)
...
mon-install: ## [Monitoring] Install/upgrade kube-prometheus-stack
	helm repo add prometheus-community https://prometheus-community.github.io/helm-charts
	helm repo update
	helm upgrade --install $(MON_REL) prometheus-community/kube-prometheus-stack \
		-n $(MON_NS) --create-namespace

mon-status: ## [Monitoring] Show monitoring components
	kubectl -n $(MON_NS) get pods,svc

mon-pf-grafana: ## [Monitoring] Port-forward Grafana :3000
...
mon-pf-prom: ## [Monitoring] Port-forward Prometheus :9090
...
mon-grafana-pass: ## [Monitoring] Print Grafana admin password
	@kubectl -n $(MON_NS) get secret $(MON_REL)-grafana -o jsonpath='{.data.admin-password}' | base64 -d; echo

mon-pf-am: ## [Monitoring] Port-forward Alertmanager :9093
...
mon-alerts-apply: ## [Monitoring] Apply alertmanager values (Slack)
	helm upgrade --install $(MON_REL) prometheus-community/kube-prometheus-stack \
		-n $(MON_NS) -f deploy/monitoring/values-alerting.yaml -f deploy/monitoring/values-extra.yaml

mon-alerts-status: ## [Monitoring] Check AM routes/receivers
	kubectl -n $(MON_NS) get pods,svc | grep alertmanager || true

# ---- Day21 helpers ----
mon-fire-crash: ## [Monitoring] Trigger Crash LoopBackOff on API (sets env.FAIL_HEALTHZ=true)
	@IP=$$(minikube ip); \
...
mon-heal-crash: ## [Monitoring] Heal CrashLoop (sets env.FAIL_HEALTHZ=false)
	@IP=$$(minikube ip); \
	helm upgrade --install $(HELM_NAME) $(HELM_DIR) -n $(HELM_NS) \
	  -f $(HELM_DIR)/values.yaml -f $(HELM_DIR)/values-dev.yaml \
...
mon-fire-cpu: ## [Monitoring] Generate CPU load via /burn
...
# ----- Security ------
sec-apply: ## [Security] Apply security hardening (NP/PDB/PriorityClass via Helm)
	make helm-up-dev-atomic

sec-test: ## [Security] Quick NP test: block external HTTP, allow Redis & DNS
...
sec-off: ## [Security] Disable default-deny (for troubleshooting) - not recommended for long
	helm upgrade --install $(HELM_NAME) $(HELM_DIR) -n $(HELM_NS) \
	  -f $(HELM_DIR)/values.yaml -f $(HELM_DIR)/values-dev.yaml \
...
redis-backup: ## [Redis] Backup Redis to ./backups/
	@NS=$(REDIS_NS) OUT_DIR=$(BACKUPS_DIR) bash scripts/redis-backup.sh

redis-list-backups: ## [Redis] List local backups
	@ls -lht $(BACKUPS_DIR)/redis-backup-*.rdb 2>/dev/null || echo "No backups found in $(BACKUPS_DIR)/"

redis-restore: ## [Redis] Restore Redis (BACKUP=./backups/redis-backup-*.rdb)
